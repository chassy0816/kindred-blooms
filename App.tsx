...
import { Sparkles, BookOpen, Calculator, Heart, Palette, Leaf, Play, Volume2, Download, RefreshCw, ChevronRight, Bookmark, Trash2, Search, X, Target, Clock, Home, Printer, Layers, Info, BarChart, Camera, Zap, ShieldCheck, Wrench, HelpCircle, Youtube, Globe, UserCheck, BookMarked } from 'lucide-react';
import { Activity, Subject, AgeGroup, ActivityDuration, Material, AppMode, LessonPlan, DifficultyLevel, ResourcesAndSupport } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem('kindred_blooms_favorites', JSON.stringify(favorites));
  }, [favorites]);
  ...
  const handleDownload = () => {
    const data = activity || lessonPlan;
    if (!data) return;

    let content = "";
    if (activity) {
      content = `ACTIVITY: ${activity.label}\nDIFFICULTY: ${activity.difficulty}\nSKILL: ${activity.primarySkill}\nTIME: ${activity.duration}\nAGE: ${activity.ageGroup}\n\nMODIFICATIONS:\n${activity.modifications?.map(m => `- ${m.type}: ${m.suggestion}`).join('\n')}\n\nMATERIALS:\n${activity.materials.map(m => `- ${m.name}`).join('\n')}\n\nSTEPS:\n${activity.steps.map((s, i) => `${i+1}. ${s}`).join('\n')}`;
    } else if (lessonPlan) {
      content = `LESSON PLAN: ${lessonPlan.title}\nDIFFICULTY: ${lessonPlan.difficulty}\nSKILL: ${lessonPlan.primarySkill}\nOBJECTIVE: ${lessonPlan.learningObjective}\n\n${lessonPlan.activities.map((a, i) => `ACTIVITY ${i+1}: ${a.label}\nMODS: ${a.modifications?.map(m => m.suggestion).join(' ')}\n${a.steps.join('\n')}`).join('\n\n')}`;
    }
    ...
    link.href = url;
    link.download = `kindred_blooms_export.txt`;
    link.click();
  };
  ...
  const toggleFavorite = (act?: Activity) => {
    ...
    if (isFav) setFavorites(favorites.filter(f => f.label !== target.label));
    else setFavorites([...favorites, target]);
  };
      ...
      <div className="grid sm:grid-cols-2 gap-5">
            ...
            {resources.youtubeVideos.map((v, i) => (
              <li key={i}>
                <a
                  ...
                  className="group flex items-start gap-2 text-sm text-slate-700 hover:text-rose-600 transition-colors"
                >
            ...
            {resources.webResources.map((r, i) => (
              <li key={i}>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-start gap-2 text-sm text-slate-700 hover:text-indigo-600 transition-colors"
                >
        ...
        <div className="bg-white p-5 rounded-2xl border border-violet-100 shadow-sm">
          ...
          <div className="space-y-3">
            <a
              href="https://www.teacherspayteachers.com/store/the-colorful-minds-hub"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-bold rounded-xl transition-colors"
            >
              <Search size={14} /> Search Colorful Minds on TPT
            </a>
            <a
              href={`https://www.pinterest.com/search/pins/?q=${encodeURIComponent(`${topic} printable${ageGroup ? ` age ${ageGroup}` : ''} activity worksheet`)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-sm font-bold rounded-xl border border-emerald-200 transition-colors"
            >
        ...
        {/* Professional Contacts — only shown when populated */}
              ...
              {resources.professionalContacts.map((c, i) => (
                <li key={i}>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group flex items-start gap-2 text-sm text-amber-800 hover:text-amber-600 transition-colors"
                  >
  ...
  useEffect(() => { if (hasStarted) handleGenerate(); }, [hasStarted]);
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row text-slate-800">
      <aside className="w-full md:w-80 bg-white border-b md:border-b-0 md:border-r border-slate-100 p-6 flex flex-col shrink-0 overflow-y-auto max-h-screen">
        <div className="flex items-center gap-2 mb-8">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex flex-col leading-tight">
              <span className="text-lg font-extrabold tracking-tight" style={{color:'#1e1b4b'}}>Kindred</span>
              <span className="text-lg font-extrabold tracking-tight" style={{color:'#6366f1'}}>Blooms</span>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button
              ...
              title="Back to home"
              className="w-9 h-9 flex items-center justify-center rounded-full text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all"
            >
              <Home size={18} />
            </button>
            <button
              onClick={() => setShowHelpModal(true)}
              title="How to use Kindred Blooms"
              className="w-9 h-9 flex items-center justify-center rounded-full text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all"
            >
              <HelpCircle size={20} />
            </button>
        ...
        <div className="space-y-6 flex-1">
          <section>
            ...
            <div className="grid grid-cols-2 gap-2 p-1 bg-slate-50 rounded-xl">
              {(['Single', 'LessonPlan'] as AppMode[]).map(m => (
                <button
                  key={m}
                  onClick={() => { setMode(m); if (m !== 'Scanner') stopScanner(); }}
          ...
          <section>
            <button
              ...
              title="Point your camera at items in your classroom — Kindred Blooms will build a custom activity around whatever it sees"
          ...
          {favorites.length > 0 && (
            <section className="pt-4 border-t border-slate-100">
              <label className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3 block">My Favorites</label>
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {favorites.map((fav, i) => (
                  <div key={i} onClick={() => {setMode('Single'); setActivity(fav); setLessonPlan(null);}} className="group flex items-center gap-2 p-2 rounded-lg bg-slate-50 hover:bg-indigo-50/30 cursor-pointer">
                    <div className="w-8 h-8 rounded-md bg-white border border-slate-100 flex items-center justify-center text-indigo-500"><Bookmark size={14} fill="currentColor" /></div>
                    <span className="text-xs font-medium truncate">{fav.label}</span>
                  </div>
                ))}
              </div>
            </section>
        ...
        <button
          ...
          disabled={isLoading || mode === 'Scanner'}
          title={mode === 'Single' ? 'Generate a single activity based on your settings' : 'Generate a multi-step mini lesson plan based on your settings'}
        ...
        >
          ...
          {isLoading ? 'Creating Magic...' : `Generate ${mode === 'Single' ? 'Activity' : 'Lesson Plan'}`}
        </button>
      </aside>

      <main className="flex-1 p-6 md:p-12 overflow-y-auto bg-[#fcfaf7]">
        {mode === 'Scanner' && isScannerOpen ? (
            ...
            <div className="relative w-full aspect-square rounded-[3rem] overflow-hidden border-8 border-white shadow-2xl bg-black">
              ...
              <div className="absolute bottom-6 left-0 right-0 flex justify-center px-8">
                <button 
                  ...
                  className="w-20 h-20 rounded-full border-4 border-white bg-indigo-500 flex items-center justify-center text-white shadow-xl hover:scale-110 active:scale-95 transition-all"
                >
                  <Zap size={32} fill="currentColor" />
                </button>
              </div>
              <button 
                onClick={() => { stopScanner(); setMode('Single'); }}
                className="absolute top-6 right-6 w-10 h-10 rounded-full bg-black/40 text-white flex items-center justify-center hover:bg-black/60 transition-all"
              >
                <X size={24} />
              </button>
            </div>
          ...
          <div className="max-w-3xl mx-auto space-y-8">
            <div className="h-10 w-48 shimmer rounded-lg"></div>
            <div className="aspect-video w-full shimmer rounded-3xl"></div>
            <div className="h-40 w-full shimmer rounded-xl"></div>
          </div>
        ) : activity ? (
          ...
          <div className="max-w-4xl mx-auto print:max-w-full">
            <header className="mb-10 text-center md:text-left">
              ...
              <div className="flex flex-col md:flex-row gap-6 p-6 bg-white rounded-[2rem] border border-slate-100 shadow-sm mb-12">
                {lessonPlan.imageUrl && (
                  <img src={lessonPlan.imageUrl} className="w-full h-56 md:w-64 md:h-64 object-cover rounded-2xl shrink-0" alt="Lesson Plan" />
                )}
                <div className="flex-1 min-w-0 space-y-4">
                  <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900 leading-snug break-words">{lessonPlan.title}</h2>
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => window.print()} className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white rounded-full text-sm font-bold hover:bg-slate-800 transition-all"><Printer size={18} /> Print Entire Plan</button>
                    <button onClick={handleDownload} className="flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-full text-sm font-bold hover:bg-slate-50 transition-all"><Download size={18} /> Download</button>
                  </div>
                </div>
              </div>
            </header>
                        ...
                        <div className="p-5 bg-slate-50 rounded-2xl">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {act.materials.map((m, i) => (
                              <span key={i} className="px-3 py-1 bg-white border border-slate-200 rounded-lg text-[11px] font-medium text-slate-600 shadow-sm" title={m.householdAlternative ? `Home alternative: ${m.householdAlternative}` : undefined}>
                                {m.name}
                              </span>
                            ))}
                          </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-400 animate-in fade-in zoom-in duration-1000">
            ...
            <div className="p-8">
              ...
              <div className="space-y-5">
                <div className="flex gap-4 p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 leading-relaxed">
                      Generates a single, ready-to-run activity. Pick an age group, subject, difficulty, and optional theme — then hit <strong>Generate Activity</strong>. Best for filling a single session or finding a quick engagement idea.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-4 p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 leading-relaxed">
                      Generates a full multi-step lesson plan with 3–4 connected activities, a learning objective, and inclusive scaffolding for each step. Great for planning a whole session or theme week. Hit <strong>Generate Lesson Plan</strong> to start.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-4 p-4 bg-amber-50 rounded-2xl border border-amber-100">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 leading-relaxed">
                      Point your camera at items in your classroom — crayons, blocks, paper, a book — and tap the flash button. The AI analyzes what it sees and builds a custom activity around those exact materials. No typing required.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-slate-50 rounded-2xl">
                ...
                <ul className="space-y-1.5 text-sm text-slate-500">
                  <li className="flex gap-2"><span className="text-indigo-400 font-bold">•</span> Add a theme (e.g. "Oceans" or "Autumn") to make activities feel cohesive and seasonal.</li>
                  <li className="flex gap-2"><span className="text-indigo-400 font-bold">•</span> Use the bookmark icon to save activities you love — they'll persist between sessions.</li>
                  <li className="flex gap-2"><span className="text-indigo-400 font-bold">•</span> Every activity includes household alternatives for materials, so families can extend learning at home.</li>
                  <li className="flex gap-2"><span className="text-indigo-400 font-bold">•</span> Tap the speaker icon to have any activity read aloud — helpful during circle time.</li>
                </ul>
              </div>
      ...
      <div className="md:hidden sticky bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-md border-t border-slate-100 flex gap-3 shadow-2xl">
        <button
          onClick={handleGenerate}
          disabled={isLoading || mode === 'Scanner'}
          title={mode === 'Single' ? 'Generate a single activity with the settings you chose' : 'Generate a multi-step mini lesson plan'}
        ...
        >
          ...
          {isLoading ? 'Generating…' : mode === 'Single' ? 'Generate Activity' : 'Generate Plan'}
        </button>
        <button
           onClick={startScanner}
           className="w-12 h-12 flex items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100"
        >
          <Camera size={24} />
        </button>
      </div>
